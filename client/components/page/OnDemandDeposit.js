...
export default class OnDemandDeposit extends React.Component {
    ...
    componentWillMount() {
        let preSelected = window.localStorage.getItem('userOnDemandDeposit');
        if (preSelected !== null) {
    ...
    handleOnDemandSubmit() {
        if (this.checkSubmit() !== false) {
            let passParam = {
                ...
                money : this.state.depositMoney.replace(/\,/g, '')
            };
            window.localStorage.setItem('userOnDemandDeposit', JSON.stringify(this.state));
